...
COORD Console::getMouseXY() {
	...
	GetConsoleScreenBufferInfo(m_hScreenBuffer, &csbi);
	COORD c = { 0,0 };
	c.X = MulDiv(point.x, csbi.srWindow.Right, rectangle.right);
	c.Y = MulDiv(point.y, csbi.srWindow.Bottom, rectangle.bottom);
	return c;
}
